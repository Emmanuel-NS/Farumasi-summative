...
class HealthRepositoryImpl implements HealthRepository {
  ...
  Future<List<HealthArticle>> getArticles() async {
    try {
      final snapshot = await _firestore.collection('health_tips').orderBy('createdAt', descending: true).get();
      return snapshot.docs.map((doc) => HealthArticleModel.fromSnapshot(doc)).toList();
    } catch (e) {
  ...
  Future<void> addArticle(HealthArticle article) async {
    try {
      ...
      final docRef = _firestore.collection('health_tips').doc(article.id.isEmpty ? null : article.id);
  ...
  Future<void> updateArticle(HealthArticle article) async {
    try {
      ...
      await _firestore.collection('health_tips').doc(article.id).update(model.toJson());
    } catch (e) {
