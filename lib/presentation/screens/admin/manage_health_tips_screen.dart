...
class ManageHealthTipsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) => HealthTipsBloc(healthRepository: HealthRepositoryImpl())..add(LoadHealthTips()),
      child: DefaultTabController(
        length: 3,
        child: _ManageHealthTipsView(),
      ),
    );
  }
}
...
class _ManageHealthTipsView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
          ...
          floatingActionButton: Builder(
            builder: (ctx) => FloatingActionButton( // Use ctx to access Bloc
              onPressed: () async {
  ...
  Widget _buildList(BuildContext context, List<HealthArticle> articles) {
     if (articles.isEmpty) {
        return Center(child: Text("No articles found.", style: TextStyle(color: Colors.grey[500])));
     }
           ...
           child: ListTile(
             ...
             leading: ClipRRect(
               ...
               child: article.imageUrl.isNotEmpty 
                  ...
                  : Container(width: 60, height: 60, color: Colors.grey.shade200, child: const Icon(Icons.article, color: Colors.grey)),
             ),
             ...
             subtitle: Column(
               ...
               children: [
                 ...
                 Text(article.category, style: TextStyle(fontSize: 12, color: Colors.green.shade700)),
                 Text(article.summary, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 12)),
               ],
             ),
             trailing: Row(
  ...
  void _confirmDelete(BuildContext context, HealthArticle article) {
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog( // ctx here is dialog context
        ...
        actions: [
          ...
          TextButton(
            onPressed: () {
              context.read<HealthTipsBloc>().add(DeleteHealthTip(article.id)); // Use context from outer build
              Navigator.pop(ctx);
            }, 
